...
if [ -f "$otter_conf" ]; then
    otter_conf=$(cd "$(dirname "$otter_conf")" && pwd)/$(basename "$otter_conf")
fi
...
OTTER_OPTS=(-DappName=otter-manager -Ddubbo.application.logger=slf4j "-Dlogging.config=$logback_configurationFile" "-Dspring.config.additional-location=file:$otter_conf")

if [ -f "$otter_conf" ] && [ -f "$logback_configurationFile" ]
then
